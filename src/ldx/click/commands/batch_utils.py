...
def validate_batch_exclusivity(name, index, batch_string, batch_lambda):
    ...
    Ensures that users don't mix incompatible command-line options:
    - Only one of --name or --index can be specified
    - Only one of -bs or -bl can be specified
    - Single-target and batch options cannot be mixed
