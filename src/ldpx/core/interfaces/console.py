"""Interface definition for LDPlayer Console operations.

This module defines the IConsole interface that specifies all available
operations for controlling LDPlayer emulator instances. Implementations
of this interface provide the actual command execution logic.

Type Aliases:
    SOptional: Union[T, None] - A type alias for optional values.
"""
import typing
...
class IConsole:
    """Interface defining all LDPlayer console operations.
    
    This interface specifies the complete API for interacting with LDPlayer
    emulator instances. All methods support targeting instances by name or index.
    
    Command Categories:
        - Simple execution: Commands with no parameters (rock, zoom, etc.)
        - Instance management: launch, quit, reboot, add, remove, etc.
        - App management: install, uninstall, run, kill apps
        - Configuration: modify instance settings, global settings
        - File operations: push, pull files
        - Query operations: list instances, check status, get properties
        - Recording operations: execute and manage recorded macros
    """
    # simple exec
