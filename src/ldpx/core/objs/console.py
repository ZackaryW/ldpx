"""LDPlayer Console implementation for command execution.

This module provides the Console class which implements the IConsole interface.
It handles command execution for LDPlayer emulator instances, supporting both
single and batch operations. Commands are dynamically generated based on the
interface definition and can target instances by name, index, or filters.
...
The Console class supports:
    ...
    - Batch operations (multiple instances)
...
import typing
import inspect
from ldpx.core.interfaces.console import IConsole, SOptional
from ldpx.core.models.list2meta import List2Meta
from ldpx.core.models.record import Record
from ldpx.core.objs.attr import LDAttr
from ldpx.core.utils.batch import BatchMixin
import ldpx.core.enums as E
from ldpx.core.utils.subprocess import open_detached, query


@dataclass
class Console(IConsole, BatchMixin):
    """Main implementation of the LDPlayer console interface.
    
    This class provides methods for controlling LDPlayer emulator instances
    through the ldconsole command-line tool. It supports both single-instance
    and batch operations.
    
    Attributes:
        attr: LDAttr instance containing LDPlayer installation paths.
        
    Usage:
        # Create console from user config
        attr = LDAttr.from_user()
        console = Console(attr)
        
        # Launch an instance by name
        console.launch(name="instance1")
        
        # Batch launch multiple instances
