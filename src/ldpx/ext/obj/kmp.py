"""Keyboard mapping (KMP) file manager.

This module provides the KMPFile class for managing LDPlayer keyboard mapping
files. KMP files define how keyboard inputs are mapped to touch and gesture
actions within Android apps.

KMP File Locations:
    - customizeConfigs: User-created keyboard mappings
    - recommendedConfigs: Pre-configured recommended mappings
"""
import json

from ldpx.ext.cache import AttrMixin
from ldpx.ext.models.kmp import KeyboardMapping
import os

class KMPFile(AttrMixin):
    """Manager for keyboard mapping (KMP) files.
    
    Provides methods to list, load, and save keyboard mapping configurations
